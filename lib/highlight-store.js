...
const HighlightStore = {
  ...
  add(streamerName, date, highlight) {
    if (typeof highlight.startOffset !== 'number' || typeof highlight.endOffset !== 'number') {
      throw new Error('highlight requires numeric startOffset and endOffset');
    }
    ...
    for (const existing of data.highlights) {
      ...
      if (startDist < threshold || gap < threshold) {
        ...
        if (highlight.peakTs && !existing.peakTs) existing.peakTs = highlight.peakTs;
        if (highlight.sourceFile && !existing.sourceFile) existing.sourceFile = highlight.sourceFile;
