...
const BiliAPI = {
  ...
  _getRedirectLocation(url) {
    return new Promise((resolve, reject) => {
      https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (res) => {
        res.resume(); // drain — we only need headers
        resolve(res.headers.location || null);
      }).on('error', reject);
    });
  },

  // 从用户输入解析房间号：纯数字 / live.bilibili.com 链接 / b23.tv 短链
  async resolveRoomId(rawInput) {
    ...
    if (/b23\.tv/.test(input)) {
      try {
        const shortUrl = input.startsWith('http') ? input : 'https://' + input;
        const loc = await this._getRedirectLocation(shortUrl);
