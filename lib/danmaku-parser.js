...
function decodePbStrings(buf) {
  ...
  const strings = [];
  let offset = 0;
  // 保险丝：正确解析每个字段至少前进 1 字节，超过即判定死循环
  let guard = buf.length * 2 + 16;
  while (offset < buf.length - 1 && guard-- > 0) {
    ...
    if (wireType === 0) {
      ...
      // Length-delimited — varint 长度必须按浮点累加：
      // len |= (b & 0x7f) << shift 在 shift≥32 时按 mod 32 环绕，
      // 会得到负数 len，offset += len 倒退 → 死循环（曾导致 CPU 打满/OOM）
      let len = 0, shift = 0, bytes = 0, malformed = false;
      while (offset < buf.length) {
        const b = buf[offset++];
        len += (b & 0x7f) * Math.pow(2, shift);
        shift += 7;
        if (++bytes > 10) { malformed = true; break; }  // varint 最多 10 字节
        if (!(b & 0x80)) break;
      }
      if (malformed || len < 0 || offset + len > buf.length) break;
...
function createDanmakuParser(roomId, logger) {
  ...
  function emitDanmaku(text, uid, uname, rawMsg) {
    stats.danmakuCount++;
    emit('danmaku', { text, uid, uname, timestamp: Date.now(), raw: rawMsg });
  }
    ...
    try {
      switch (msg.cmd) {
        case 'DANMU_MSG': {
          ...
          emitDanmaku(info[1] || '', user[0] || 0, user[1] || '', msg);
          break;
        }
        ...
        case 'INTERACT_WORD_V2': {
          ...
          if (d.pb) {
            ...
            // Field 12 = danmaku text (non-empty = danmaku, empty = enter-room)
            const msgField = strings.find(s => s.fieldNum === 12 && s.val);
            const unameField = strings.find(s => s.fieldNum === 2 && s.val && s.val.length < 20 && !/[\x00-\x08\x0b\x0c\x0e-\x1f]/.test(s.val));
            const text = msgField ? msgField.val : '';
            const uname = unameField ? unameField.val : '';
            if (text) emitDanmaku(text, 0, uname, msg);
          }
          break;
        }

        case 'LIKE_INFO_V3_CLICK': {
          // 点赞不再当弹幕喂给引擎：大直播间点赞是每秒数千条的洪水，
          // 会灌爆 _recentTexts / 刷屏式触发 O(n) 评估导致 OOM，
          // 且把 consistencyScore 刷满（人人都"说"点赞）造成误报
...
module.exports = { createDanmakuParser, biliGet, packAuth, decodePbStrings };
