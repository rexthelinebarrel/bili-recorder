      ...
      switch (msg.cmd) {
        ...
        case 'INTERACT_WORD_V2': {
          ...
          if (d.pb) {
            ...
            // Look for danmaku text: longest non-URL string, excluding binary/control chars
            const candidates = strings.filter(s => s.val && !/^https?:\/\//i.test(s.val) && !/^[\x00-\x1f]+$/);
            candidates.sort((a, b) => b.val.length - a.val.length);
            const text = candidates.length > 0 ? candidates[0].val.trim() : '';
            // Log first few messages (both with and without text)
            if (stats.danmakuCount < 3 && logger) {
              logger.info(`DanmakuParser: INTERACT_WORD_V2 fields: ${JSON.stringify(strings.map(s=>({f:s.fieldNum,v:s.val.slice(0,50)})))}`);
            }
            if (text) {
              const danmaku = {
                text,
              ...
              stats.danmakuCount++;
              emit('danmaku', danmaku);
              if (logger) logger.info(`DanmakuParser: danmaku text="${text.slice(0,60)}"`);
            }
          }
          break;
        }
