      ...
      switch (msg.cmd) {
        ...
        case 'INTERACT_WORD_V2': {
          ...
          if (d.pb) {
            ...
            // Log first 3 INTERACT_WORD_V2 regardless
            const logCount = (stats._interactLog || 0) + 1;
            stats._interactLog = logCount;
            if (logCount <= 3 && logger) {
              logger.info(`DanmakuParser: INTERACT_WORD_V2 hasText=${!!text} uname="${uname}" text="${text.slice(0,40)}" fields=${JSON.stringify(strings.map(s=>({f:s.fieldNum,v:s.val.slice(0,30)})))}`);
            }
            if (text) {
              const danmaku = {
                text,
        ...
        default:
          // Log new/unknown message types for debugging (first 2 of each cmd)
          stats._unknownCmds = stats._unknownCmds || {};
          stats._unknownCmds[msg.cmd] = (stats._unknownCmds[msg.cmd] || 0) + 1;
          if (stats._unknownCmds[msg.cmd] <= 2) {
            if (logger) logger.info(`DanmakuParser: unknown cmd=${msg.cmd} sample=${JSON.stringify(msg).slice(0, 600)}`);
          }
          emit('raw', msg);
          break;
      }
