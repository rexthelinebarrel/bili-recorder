 ...
 */
function parseMessages(text, logger) {
  if (!text) return [];
  const messages = [];
  ...
  if (messages.length === 0 && text.length > 0 && logger) {
    logger.warn(`DanmakuParser: parseMessages got 0 from ${text.length}B text, preview: ${text.slice(0, 300)}`);
  }
  return messages;
}
...
function createDanmakuParser(roomId, logger) {
  ...
  function handleBinary(data) {
    ...
    const text = decompressBody(packet.protoVer, packet.body, logger);
    if (!text) return;

    const messages = parseMessages(text, logger);
