  ...
  async check() {
    ...
    for (const s of streamers) {
      try {
        ...
        // 观众数采样喂给高光引擎（viewer_surge 信号）
        if (info.status === 'live' && info.online > 0) {
          const eng = DanmakuManager.getEngine(s.id);
          if (eng) eng.feedViewerCount(info.online);
        }
          ...
          if (!Recorder.isRecording(s.id)) {
            if (s.gaveUpAt && (Date.now() - s.gaveUpAt) < GIVEUP_BACKOFF) {
              ...
              if (gap <= RECONNECT_WINDOW) {
                logger.warn(`[recorder] Reconnecting ${s.name} (gap: ${Math.round(gap/1000)}s, new segment file)`);
                const realRoomId = s.realRoomId || info.roomId;
                try {
                  // 录到新分片文件（旧版复用同一路径会被 ffmpeg -y 覆盖丢数据）
                  // 高光剪切通过 segments.json 的墙钟映射定位，不依赖单文件连续
                  const newFile = await Recorder.start(s.id, realRoomId, s.name);
                  Store.updateStreamer(s.id, { recording: true, lastLiveTime: Date.now(), gaveUpAt: null, lastFilePath: newFile });
