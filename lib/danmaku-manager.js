...
const RESTDanmakuPoller = {
  ...
  _failCount: {},   // streamerId -> consecutive poll failures (log throttle)
  ...
  start(streamerId, roomId, engine) {
    ...
    const poll = async () => {
      try {
        ...
        const res = await BiliAPI._get(url, {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
          'Referer': 'https://live.bilibili.com/'
        });

        if (res.code !== 0 || !res.data) {
          // 之前这里静默 return，被风控/限流时完全无迹可循
          const n = (this._failCount[streamerId] = (this._failCount[streamerId] || 0) + 1);
          if (n === 1 || n % 100 === 0) {
            logger.warn(`[rest-danmaku] gethistory code=${res.code} for room ${roomId} (x${n})`);
          }
          return;
        }
        this._failCount[streamerId] = 0;
        ...
        for (const m of roomMsgs) {
          const tl = m.timeline || '';
          // 防御异常 timeline：格式不对或超过当前时间 5min 的（置顶/预约消息）
          // 会污染去重水位，导致后续所有弹幕被永久去重（曾因此静默断流 8 分钟）
          if (!/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(tl)) continue;
          const tlMs = new Date(tl.replace(' ', 'T')).getTime();
          if (isNaN(tlMs) || tlMs > Date.now() + 5 * 60 * 1000) continue;
          if (tl <= this._lastTime[streamerId]) continue;
          this._lastTime[streamerId] = tl;
          if (m.text) engine.feedDanmaku(m.text, m.nickname || 0);
          newCount++;
        }
        if (newCount > 0) {
        ...
        // 8s 轮询不能每次都打日志，首次失败和每 100 次报一次
        const n = (this._failCount[streamerId] = (this._failCount[streamerId] || 0) + 1);
        if (n === 1 || n % 100 === 0) {
          logger.warn(`[rest-danmaku] Poll failed x${n} for room ${roomId}: ${e.message}`);
        }
      }
    };
  ...
  stop(streamerId) {
    ...
    delete this._failCount[streamerId];
  }
};
...
const DanmakuManager = {
  ...
  start(streamerId, streamerName, roomId) {
    ...
    const emotionDict = (streamer && streamer.emotionDict) || Store.getEmotionDict() || null;
    const keywordPatterns = (streamer && streamer.keywordPatterns) || Store.getKeywordPatterns();
    const engine = createHighlightEngine(streamerId, streamerName, roomId, logger, emotionDict, keywordPatterns);
  ...
  activeIds() { return Object.keys(this._parsers); }
};
