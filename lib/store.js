...
// 关键词正则（字符串形式存 config.json，可在主播级覆盖）
...
const Store = {
  ...
  load() {
    ...
    // Ensure keywordPatterns exists
    if (!this._data.keywordPatterns) {
      this._data.keywordPatterns = DEFAULT_KEYWORD_PATTERNS.slice();
      this.save();
    }
    return this._data;
  },
  ...
  getKeywordPatterns() { return this._data.keywordPatterns || []; },

  setKeywordPatterns(patterns) {
    this._data.keywordPatterns = patterns;
    this.save();
  }
};
