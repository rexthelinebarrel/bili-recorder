...
const { Recorder, FFMPEG_BIN, isValidMP4, findBestSourceFile, VIDEO_EXTS } = require('./recorder');
      ...
      for (const [sid] of Object.entries(Recorder._processes)) {
        try {
          ...
          if (stoppedFile) {
            ...
            if (streamer) {
              try {
                await autoClipAfterStream(streamer.name, stoppedFile);
              } catch (e) {
                logger.warn(`[clip] Auto-clip failed for ${streamer.name}: ${e.message}`);
              }
            }
          }
        } catch (e) {
          logger.error(`[shutdown] Error cleaning up streamer ${sid}: ${e.message}`);
        }
      }
    ...
    try {
      for (const [sid] of Object.entries(Recorder._processes)) {
        try {
          ...
          logger.error(`[exit] Error cleaning up streamer ${sid}: ${e.message}`);
        }
      }
      for (const [sid] of Object.entries(DanmakuManager._parsers)) {
        try { DanmakuManager.stop(sid); } catch {}
